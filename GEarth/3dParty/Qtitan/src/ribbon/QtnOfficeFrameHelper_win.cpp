...
//    fillSolidRect(painter, opt.rcTitleText, QColor(0, 0, 0));
...
static QFont qtn_LOGFONTtoQFont(const LOGFONT& lf, bool /*scale*/)
{
    QString family = QString::fromWCharArray(lf.lfFaceName);
    QFont qf(family);
    qf.setItalic(lf.lfItalic);
    if (lf.lfWeight != FW_DONTCARE)
        qf.setWeight(qtn_weightFromInteger(lf.lfWeight));
    int lfh = qAbs(lf.lfHeight);
    const HDC displayDC = GetDC(0);
    qf.setPointSizeF(lfh * 72.0 / GetDeviceCaps(displayDC,LOGPIXELSY));
...
bool OfficeFrameHelperWin::getCaptionQtnFont(int dpi, QFont* result)
{
    ...
    return ni && QMetaObject::invokeMethod(ni, "logFontToQFont", Qt::DirectConnection,
                                           ...
                                           Q_ARG(int, dpi));
#else
    const HANDLE hTheme = m_hwndFrame ? pOpenThemeData(m_hwndFrame, L"WINDOW") : Q_NULL;
    if (!hTheme)
        return false;
    const LOGFONT logFont = getCaptionLogFont(isTheme2013() ? hTheme : Q_NULL);
    *result = qtn_LOGFONTtoQFont(logFont, dpi);
    return true;
#endif
}
            ...
            *result = DefWindowProcW(msg->hwnd, msg->message, msg->wParam, msg->lParam);
            ...
            if (*result == HTCLIENT)
            {
                ...
                if (*result == HTCLIENT)
                {
                    ...
                    if (m_ribbonBar && m_rcHeader.isValid())
                    {
                        if (!hitTestContextHeaders(pos) && !hitTestBackstageHeaders(pos) && m_rcHeader.contains(pos))
                        {
                            if ((GetAsyncKeyState(::GetSystemMetrics(SM_SWAPBUTTON) ? VK_RBUTTON : VK_LBUTTON) & 0x8000) != 0)
                                *result = HTCAPTION;
                            else if ((GetAsyncKeyState(::GetSystemMetrics(SM_SWAPBUTTON) ? VK_LBUTTON : VK_RBUTTON) & 0x8000) != 0)
                                ::SendMessage(m_hwndFrame, 0x0313, (WPARAM)m_hwndFrame, MAKELPARAM(point.x, point.y));
                        }
                    }
                    else
                    {
                    ...
                    {
                        if (pos.x() < 7 + iconSize() && (getStyle()& WS_SYSMENU))
                            *result = HTSYSMENU;
                    }
                }
            }
    ...
    else if (m_hwndFrame != Q_NULL && msg->message == WM_NCHITTEST  && !isTitleVisible() && m_dwmEnabled)
    {
        ...
        if (*result == HTCLIENT || *result == HTSYSMENU)
        {
            ...
            if (::PtInRect(&rc, point))
            {
                ...
                if (*result != HTSYSMENU)
                {
                    if (pos.x() < m_frameBorder + iconSize() && (getStyle() & WS_SYSMENU))
                        *result = HTSYSMENU;
                    else if ((GetAsyncKeyState(::GetSystemMetrics(SM_SWAPBUTTON) ? VK_RBUTTON : VK_LBUTTON) & 0x8000) != 0)
                        *result = HTCAPTION;
                    else if ((GetAsyncKeyState(::GetSystemMetrics(SM_SWAPBUTTON) ? VK_LBUTTON : VK_RBUTTON) & 0x8000) != 0)
                        ::SendMessage(m_hwndFrame, 0x0313, (WPARAM)m_hwndFrame, MAKELPARAM(point.x, point.y));
                }
