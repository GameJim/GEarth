...
QSize RibbonButton::sizeHint() const
{
    ...
    {
        //由于直接new RibbonButtonControl(),会导致崩溃；所以把这段代码注释了，换成了下面的逻辑,
//         if (RibbonBar* ribbonBar = control->parentGroup()->ribbonBar())
//             sz.setHeight(ribbonBar->rowItemHeight());
        //[gcmp, 2019-04-12 penglj]
        RibbonBar* ribbonBar = control->parentGroup() ? control->parentGroup()->ribbonBar() : nullptr;
        if (ribbonBar)
            sz.setHeight(ribbonBar->rowItemHeight());
        //[end gcmp]
    }
