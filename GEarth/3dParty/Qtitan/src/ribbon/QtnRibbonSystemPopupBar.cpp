...
static const int systemPopupMargin       = 6;
...
void RibbonSystemButton::paintEvent(QPaintEvent* event)
{
    ...
    if (!opt.icon.isNull())
    {
        QPixmap pm = opt.icon.pixmap(iconSize());
...
{
    m_rcBorders = QRect(QPoint(systemPopupBorderLeft, systemPopupBorderTop), QPoint(systemPopupBorderRight, systemPopupBorderBottom));
}
...
RibbonPageSystemPopup* RibbonSystemPopupBar::addPageSystemPopup(const QString& caption, QAction* defaultAction, bool splitAction)
{
    ...
    if (splitAction)
        defaultAction->setObjectName(_qtn_SplitActionPopup);
                    ...
                    if (rectMenu.isValid())
                    {
                        ...
                        if (posButton.y() < posMenu.y())
                        {
                            ...
                            if (ribbonBar->tabBarPosition() != RibbonBar::TabBarBottomPosition)
                            {
                                ...
                                move(QPoint(posMenu.x(), posParent.y() + ribbonBar->titleBarHeight() + RibbonBarPrivate::_get(ribbonBar)->topBorder()));
                            }
                            else
                            {
                                QPoint posParent = ribbonBar->mapToGlobal(ribbonBar->pos());
                                RibbonSystemButton* systemButton = ribbonBar->getSystemButton();
                                move(QPoint(posMenu.x(), posParent.y() + systemButton->geometry().top() + systemButton->geometry().height() / 2 + RibbonBarPrivate::_get(ribbonBar)->topBorder()));
                            }
                        }
                    }
                }
...
void RibbonSystemPopupBar::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    if (action && (action->objectName() == QString(_qtn_SplitActionPopup)))
    {
...
void RibbonSystemPopupBar::paintEvent(QPaintEvent* event)
{
    ...
    for (int i = 0; i < actionList.count(); ++i) 
    {
        ...
        if (action->objectName() == QString(_qtn_SplitActionPopup))
        {
            opt.text = QString(_qtn_SplitActionPopup) + opt.text;
    ...
    if (count > 0)
    {
        ...
        p.fillRect(QRect(QPoint(sz.width() + d.m_rcBorders.left() - hMargin, d.m_rcBorders.top()), 
            QSize(1, sz.height() - d.m_rcBorders.top() - d.m_rcBorders.bottom() * 0.5 - 2)), fillColor);
        ...
        if (hintRectReturn.isValid)
        {
            QRect rect = QRect(QPoint(d.m_rcBorders.left(), d.m_rcBorders.top()), 
                QSize(sz.width() - hMargin, sz.height() - d.m_rcBorders.top() - d.m_rcBorders.bottom() * 0.5 - 2) );
    ...
    for (int i = 0; i < count; i++)
    {
        if (RibbonPageSystemPopupListCaption* page = qobject_cast<RibbonPageSystemPopupListCaption*>(d.m_pageList.at(i)))
        {
            ...
            {
                QRect rect = page->geometry().adjusted(0, 0, 0, 1);
                QPainterPath path;
    ...
    if (RibbonBar* parent = qobject_cast<RibbonBar*>(parentWidget()))
    {
        if (RibbonSystemButton* button = parent->getSystemButton())
        {
            if (button->toolButtonStyle() == Qt::ToolButtonFollowStyle)
            {
                QStyleOptionToolButton opt;
...
{
    int height = systemPopupBorderTop;
    ...
    if (!strTitle.isEmpty())
    {
        ...
        QRect rc = fm.boundingRect(strTitle);
        height += rc.height();
    }
...
void RibbonPageSystemPopupListCaption::paintEvent(QPaintEvent* event)
{
    ...
    QRect rcTextMetrics = fm.boundingRect(windowTitle());

    QRect rc = opt.rect;
    QRect rcText(rc.left() + 7, rc.top(), rc.right(), (rc.top() + rcTextMetrics.height() + systemPopupMargin));

    StyleHintReturnThemeColor hintClientRight("Ribbon", "SysPopupRight");
    const QColor fill = static_cast<QRgb>(style()->styleHint(QStyle::SH_CustomBase, Q_NULL, this, &hintClientRight));
    if (hintClientRight.isValid)
        p.fillRect(rc, fill);
    ...
    p.fillRect(QRect(QPoint(rc.left(), rcText.bottom() + 0), QSize(rc.width(), 1)), fillEdgeShadow);
    ...
    p.fillRect(QRect(QPoint(rc.left(), rcText.bottom() + 1), QSize(rc.width(), 1)), edgeHighLightColor);
...
void RibbonPageSystemRecentFileListPrivate::updateActionRects() const
{
    ...
    int y = p.sizeHint().height() - systemPopupBorderTop + systemPopupMargin + 4;
    ...
    : QMenu(parent)
{
    ...
    QString text = QString(_qtn_PopupLable) + d.m_caption;
...
void RibbonPageSystemPopup::paintEvent(QPaintEvent* event)
{
    ...
    for (int i = 0; i < actionList.count(); ++i) 
    {
        ...
        if (opt.text.count(QString(_qtn_PopupLable)) == 0)
...
void RibbonPageSystemPopup::leaveEvent(QEvent* event)
{
    Q_UNUSED(event);
}
