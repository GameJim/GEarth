...
static inline void validateCustomBarArea(LayoutArea& area)
{
    switch (area) 
    {
...
QSize CustomLayout::sizeHint() const
{
    if (!m_szHint.isValid())
    {
        ...
        const QSize cwHint = m_centralWidget ? m_centralWidget->sizeHint() : QSize(0, 0);
...
QSize CustomLayout::minimumSize() const
{
    if (!m_minSize.isValid()) 
    {
        ...
        const QSize cwMin = m_centralWidget ? m_centralWidget->minimumSize() : QSize(0, 0);
