...
__inline void NormalizeRect(RECT& r)
{
    int nTemp;
    if (r.left > r.right)
    {
...
__inline int RectWidth(const RECT &rIn)
{
    RECT r;
    CopyRect(&r, &rIn);
    NormalizeRect(r);
...
__inline int RectHeight(const RECT &rIn)
{
    RECT r;
    CopyRect(&r, &rIn);
    NormalizeRect(r);
...
bool qtn_isCompositionAvailable();
bool qtn_isWindows10();
QPixmap qtn_createPixmap(HBITMAP bitmap, int hbitmapFormat = 1 /* PremultipliedAlpha */);
HBITMAP qtn_createHBITMAP(const QPixmap& pixmap, int hbitmapFormat = 1 /* PremultipliedAlpha */);
HICON qtn_createHICON(const QIcon& icon, QSize& inOutSize);
HDC qtn_getDC(QPainter* painter);
void qtn_releaseDC(QPainter* painter, HDC dc);
HDC qtn_createDC();
