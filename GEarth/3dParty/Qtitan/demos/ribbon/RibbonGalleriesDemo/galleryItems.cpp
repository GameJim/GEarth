...
void GalleryItemTable::draw(QPainter* p, Qtitan::RibbonGallery* gallery, QRect rcItem, bool enabled, bool selected, bool pressed, bool checked)
{
    ...
    int indexItem = gallery->isItemSelected() ? gallery->selectedItem() : -1;
    QRect rcItems = gallery->getItemsRect();
    QSize szItems = gallery->galleryGroup()->size();
...
{
    int item = gallery->isItemSelected() ? gallery->selectedItem() : -1;
...
static QRgb translateColor(const QRgb& clr, int transalte)
{
    return transalte == 0 ? clr : transalte > 0 ? Qtitan::DrawHelpers::lightColor(clr, transalte) : Qtitan::DrawHelpers::darkColor(clr, transalte);
}

static QString getColorTip(const QString& strToolTip, const QString& strAccent, int transalte)
{
    QString strTranslate = strToolTip;
    strTranslate += QObject::tr(", %1").arg(transalte > 0 ? QObject::tr("Lighter") : QObject::tr("Darker"));
    strTranslate += QObject::tr(" %1%").arg(abs(transalte));
    return (!strToolTip.isEmpty() ? strToolTip + QObject::tr(", ") : QString(QObject::tr(""))) + strAccent + (transalte != 0 ? strTranslate : QObject::tr(""));
}

void GalleryItemFontColor::addThemeColors(RibbonGalleryGroup* items, int theme)
{
    const COLORPREVIEW& clrPreview = themeColors[theme];

    for (int j = 0; j < 6; j++)
    {
        bool topMargin = j < 2;
        bool bottomMargin = j == 0 || j == 5;

        for (int i = 0; i < 10; i++)
        {
            int translate = matrix[j][i];
            QColor clr = translateColor(clrPreview.rec[i].clr.rgb(), translate);
            QString strToolTip = getColorTip(clrPreview.rec[i].caption, strMatrixTip[i], translate);

            items->appendItem(new GalleryItemFontColor(clr, strToolTip, topMargin, bottomMargin));
        }
    }
}
...
void WidgetGalleryUndo::selectedItemChanged()
{
    if (QLabel* label = findInfoWidget())
        label->setText(isItemSelected() ? QObject::tr("Undo %1 Actions").arg(selectedItem() + 1) : QObject::tr("Cancel"));
}
