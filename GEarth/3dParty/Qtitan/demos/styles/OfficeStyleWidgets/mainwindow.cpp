...
#include "ui_OfficeStyleDialog.h"
...
void Widgets::createActions()
{
    ...
    m_actOfficeStyle = new QAction(tr("Office"), this);
    m_actOfficeStyle->setStatusTip(tr("Office Style"));
    m_actOfficeStyle->setCheckable(true);
    groupActions->addAction(m_actOfficeStyle);
...
void Widgets::createMenubar()
{
    ...
    m_menuView->addAction(m_actOfficeStyle);
    m_menuView->addSeparator();
...
void Widgets::createToolbar()
{
    ...
    customBar->addAction(m_actOfficeStyle);
...
void Widgets::createStyleDialog()
{
    ...
    button2->move(button1->sizeHint().width() + 20, 10);
...
void Widgets::updateTheme(QAction* act)
{
    ...
    if (m_officeStyle)
    {
        ...
        else if (act == m_act2013GrayTheme)
            themeId = Qtitan::OfficeStyle::Office2013Gray;
        else if (act == m_act2013DarkTheme)
            themeId = Qtitan::OfficeStyle::Office2013Dark;
        else if (act == m_act2016ColorfulTheme)
            themeId = Qtitan::OfficeStyle::Office2016Colorful;
        else if (act == m_act2016WhiteTheme)
            themeId = Qtitan::OfficeStyle::Office2016White;
        else if (act == m_act2016DarkGrayTheme)
            themeId = Qtitan::OfficeStyle::Office2016DarkGray;
        else if (act == m_act2016BlackTheme)
...
void Widgets::about()
{
    Qtitan::AboutDialog::show(this, tr("About Qtitan Widgets"), 
        tr("OfficeStyle"), QLatin1String(QTN_VERSION_RIBBON_STR));
}
