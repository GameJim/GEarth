...
bool OfficePaintManager::drawTitleBar(const QStyleOptionComplex* opt, QPainter* p, const QWidget* wd) const
{
    ...
    {
        const bool active = tb->titleBarState & QStyle::State_Active;
        QRect ir;
...
bool OfficePaintManager::drawIndicatorDockWidgetResizeHandle(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    const QRect rcFrame = w->rect();
    const float fHeight = static_cast<float>(rcFrame.height());

    const QRgb clr1 = DrawHelpers::blendColors(d.m_palSplitter.color(QPalette::Light).rgb(), 
        d.m_palSplitter.color(QPalette::Dark).rgb(), static_cast<float>(opt->rect.bottom() - rcFrame.top()) / fHeight);

    const QRgb clr2 = DrawHelpers::blendColors(d.m_palSplitter.color(QPalette::Light).rgb(), 
        d.m_palSplitter.color(QPalette::Dark).rgb(), static_cast<float>(opt->rect.top() - rcFrame.top()) / fHeight);
...
bool OfficePaintManager::drawPanelMenu(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if(showGripper)
    {
        ...
        const int iconSize = baseStyle()->pixelMetric(QStyle::PM_ToolBarIconSize, opt, w) + 2;
...
// for TabBar
/*! \internal */
bool OfficePaintManager::drawFrameTabWidget(const QStyleOption* opt, QPainter* p, const QWidget*) const
{
...
/*! \internal */
bool OfficePaintManager::drawFrameTabBarBase(const QStyleOption*, QPainter*, const QWidget*) const
{
    return false;
}
...
bool OfficePaintManager::drawTabBarTabShape(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    if (const QStyleOptionTab* tab = qstyleoption_cast<const QStyleOptionTab*>(opt))
    {
        ...
        const bool correct = w != 0 ? qobject_cast<QMdiArea*>(w->parentWidget()) != 0 : true;
        const int borderThickness = baseStyle()->proxy()->pixelMetric(QStyle::PM_DefaultFrameWidth, opt, w)/2;
...
bool OfficePaintManager::drawIndicatorTabClose(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    const bool active = opt->state & QStyle::State_Raised;
    const bool down = opt->state & QStyle::State_Sunken;
    const bool highlighted = opt->state & QStyle::State_MouseOver;

    const int state = !active ? 3 : down && highlighted ? 2 : highlighted || down ? 1 : 0;
    QRect rcSrc = sourceRectImage(pm.rect(), state, 5);
    QRect rcGlyph(QPoint((ir.right() + ir.left() - rcSrc.width()+1) / 2, (ir.top() + ir.bottom() - rcSrc.height()+2) / 2), rcSrc.size());
...
// for QForm
/*! \internal */
bool OfficePaintManager::drawFrame(const QStyleOption*, QPainter*, const QWidget*) const
{
    return false;
}
...
bool OfficePaintManager::drawFrameWindow(const QStyleOption* opt, QPainter* p, const QWidget* wd) const
{
    ...
    if (const QStyleOptionFrame* frm = qstyleoption_cast<const QStyleOptionFrame *>(opt))
    {
        ...
        {
            QStyle::State flags = opt->state;
            const bool active = flags & QStyle::State_Active;
...
bool OfficePaintManager::drawScrollBarLine(QStyle::ControlElement element, const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionSlider* scrollbar = qstyleoption_cast<const QStyleOptionSlider*>(opt))
    {
        const bool light = w && (qobject_cast<const RibbonGallery*>(w->parentWidget()) ||
            qobject_cast<const QDialog*>(w->topLevelWidget()) || ::isAncestor(w));
...
bool OfficePaintManager::drawPanelItemViewItem(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    #endif
    {
        ...
        const bool enabled  = vopt->state & QStyle::State_Enabled;
        const bool selected = vopt->state & QStyle::State_Selected;
        const bool hasFocus = vopt->state & QStyle::State_HasFocus;
        bool over = vopt->state & QStyle::State_MouseOver;
        ...
        int state = -1;
        if ((selected || over) && enabled)
            state = selected && over? 2 : hasFocus && selected ? 1 : !hasFocus && selected ? 3 : 0;
        ...
        case QStyle::PE_IndicatorArrowLeft:
            {
                if (pe == QStyle::PE_IndicatorArrowRight && qstyleoption_cast<const QStyleOptionMenuItem*>(opt))
                {
                    ...
                    const int size = rcSrc.height();
                    const int xOffset = opt->rect.x() + (opt->rect.width() -  size)/2;
                    const int yOffset = opt->rect.y() + (opt->rect.height() - size)/2;
...
bool OfficePaintManager::pixelMetric(QStyle::PixelMetric pm, const QStyleOption* opt, const QWidget* w, int& ret) const
{
    ...
    switch(pm)
    {
        case QStyle::PM_MenuButtonIndicator:
            if (qobject_cast<const QToolButton*>(w) && !qobject_cast<const QTabBar*>(w->parentWidget()))
            {
                ret = baseStyle()->property("DPIAware").toBool() ? int(DrawHelpers::dpiScaled(qreal(iImageWidth))) : iImageWidth;
                return true;
            }
            break;
        default:
...
void OfficePaintManager::drawMenuItemSeparator(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    if (const QStyleOptionMenuItem* menuitem = qstyleoption_cast<const QStyleOptionMenuItem*>(opt)) 
    {
        ...
        else
        {
            ...
            const int text_flags = Qt::AlignVCenter | Qt::TextSingleLine;
...
//    m_ImagesWhite  = mapTo3dColors(m_ImagesSrc, QColor(0, 0, 0), QColor(255, 255, 255));
    m_ImagesWhite = mapTo3dColors(m_ImagesSrc, QColor(0, 0, 0), QColor(192, 192, 192));
...
bool OfficePaintManager2013::drawIndicatorMenuCheckMark(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    OfficePaintManager2013::ImageState imageState = black ? OfficePaintManager2013::White : OfficePaintManager2013::Gray;
                                                           ...
                                                           QPainter* p, const QWidget* widget) const
{
    ...
    const bool isRadio = (element == QStyle::PE_IndicatorRadioButton);
...
bool OfficePaintManager2013::drawSpinBox(const QStyleOptionComplex* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionSpinBox* sb = qstyleoption_cast<const QStyleOptionSpinBox *>(opt))
    {
        const bool highlighted = sb->state & QStyle::State_MouseOver;
        const bool enabled  = sb->state & QStyle::State_Enabled;
        const bool hasFocus = sb->state & QStyle::State_HasFocus;
...
// for TabBar
/*! \internal */
bool OfficePaintManager2013::drawFrameTabWidget(const QStyleOption* opt, QPainter* painter, const QWidget*) const
{
    QTN_D_STYLE(OfficeStyle)
    painter->fillRect(opt->rect.adjusted(0, 0, -1, -1), d.m_clrDockBar);
    if (const QStyleOptionTabWidgetFrame* twf = qstyleoption_cast<const QStyleOptionTabWidgetFrame *>(opt))
    {
        const int borderThickness = 1;//baseStyle()->proxy()->pixelMetric(QStyle::PM_DefaultFrameWidth, option, widget);
        QRect rect = twf->rect.adjusted(0, 0, -1, -1);
        QPen borderColor(m_clrBarDkShadow, borderThickness);
...
/*! \internal */
bool OfficePaintManager2013::drawFrameTabBarBase(const QStyleOption* option, QPainter* p, const QWidget* widget) const
{
    if (const QStyleOptionTabBarBase* tbb = qstyleoption_cast<const QStyleOptionTabBarBase *>(option)) 
    {
        ...
        switch (tbb->shape) 
        {
            ...
            case QTabBar::RoundedWest:
                ...
                break;
            case QTabBar::RoundedSouth:
                    p->drawLine(tbb->rect.left(), tbb->rect.bottom(), tbb->rect.right(), tbb->rect.bottom());
                break;
            case QTabBar::RoundedEast:
                    p->drawLine(tbb->rect.topRight() + QPoint(-2, 0), tbb->rect.bottomRight() + QPoint(-2, 0));
                break;
            case QTabBar::TriangularNorth:
            case QTabBar::TriangularEast:
            case QTabBar::TriangularWest:
            case QTabBar::TriangularSouth:
                    p->restore();
                    baseStyle()->QCommonStyle::drawPrimitive(QStyle::PE_FrameTabBarBase, option, p, widget);
                return true;
        }
        p->restore();
        return true;
...
/*! \internal */
bool OfficePaintManager2013::drawTabBarTabShape(const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    if (const QStyleOptionTab* tab = qstyleoption_cast<const QStyleOptionTab *>(option))
    {
        ...
        switch (tab->shape) 
        {
            ...
            default:
                painter->restore();
                baseStyle()->QCommonStyle::drawControl(QStyle::CE_TabBarTabShape, tab, painter, widget);
                return true;
        }
        ...
        QColor tabFrameColor = selected ? m_clrBarHilite : m_clrHighlightMenuItem;

        QRect drawRect = rect.adjusted(0, selected ? 0 : 2, 0, 3);
        if (selected || isHot)
        {
            painter->setPen(m_clrBarDkShadow);
            painter->save();
            painter->setClipRect(rect.adjusted(-1, -1, 1, selected ? -2 : -3));
            painter->setBrush(tabFrameColor);
            painter->drawRoundedRect(drawRect.adjusted(0, 0, -1, -1), 0.0, 0.0);
            painter->restore();
        }
...
// for QForm
/*! \internal */
bool OfficePaintManager2013::drawFrame(const QStyleOption* option, QPainter* painter, const QWidget*) const
{
    QPen savePen = painter->pen();
    QPen thePen(m_clrBarDkShadow);
            ...
            case QFrame::Panel:
                {
                    if (frameShadow == QFrame::Sunken)
                    {
                        ...
                        if (QTabBar* tabBar = mdiArea->findChild<QTabBar*>())
                        {
                            ...
                            {
                                case QTabWidget::North:
                                    rect.adjust(0, tabBar->sizeHint().height() - 1, -1, -1);
                                    break;
                                case QTabWidget::South:
                                    rect.adjust(0, 0, -1, -tabBar->sizeHint().height() + 1);
                                    break;
                                case QTabWidget::West:
                                    rect.adjust(tabBar->sizeHint().width() - 1, 0, -1, -1);
                                    break;
                                case QTabWidget::East:
                                    rect.adjust(0, 0, -1, -1);
                                    break;
...
bool OfficePaintManager2013::drawScrollBarSlider(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionSlider* scrollbar = qstyleoption_cast<const QStyleOptionSlider*>(opt)) 
    {
        if (!scrollbar->rect.isEmpty())
        {
            ...
            const bool highlighted = scrollbar->state & QStyle::State_MouseOver;
...
/*! \internal */
bool OfficePaintManager2013::drawIndicatorHeaderArrow(const QStyleOption* opt, QPainter* p, const QWidget*) const
{
    if (const QStyleOptionHeader* header = qstyleoption_cast<const QStyleOptionHeader*>(opt))
    {
        QPoint pt(opt->rect.center());
        ...
        {
            DrawHelpers::drawTriangle(*p, QPoint(pt.x() - 4, pt.y() - 2),
                QPoint(pt.x(), pt.y() + 2), QPoint(pt.x() + 4, pt.y()  - 2), opt->palette.dark().color());
        }
        else if (header->sortIndicator == QStyleOptionHeader::SortDown)
        {
            DrawHelpers::drawTriangle(*p, QPoint(pt.x() - 4, pt.y() + 2),
...
void OfficePaintManager2013::drawIcon(QPainter* p, const QRect& rectImage, ImageIcons index, ImageState state, const QSize& sizeImageDest) const
{
    ...
    if (baseStyle()->property("DPIAware").toBool())
    {
        QSize sizeDpiImage = QSize(static_cast<int>(DrawHelpers::dpiScaled(static_cast<qreal>(sizeImage.width()))), 
            static_cast<int>(DrawHelpers::dpiScaled(static_cast<qreal>(sizeImage.height()))));
        QPoint ptDpiImage(rectImage.left() + (rectImage.width() - sizeDpiImage.width()) / 2 + ((rectImage.width() - sizeDpiImage.width()) % 2),
            rectImage.top() + (rectImage.height() - sizeDpiImage.height()) / 2 + ((rectImage.height() - sizeDpiImage.height()) % 2));
        ptImage.setX(ptDpiImage.x()); ptImage.setY(ptDpiImage.y());
    }
...
void OfficePaintManager2013::drawIcon(QPainter* p, const QPoint& pos, ImageIcons index, ImageState state, const QSize& sz) const
{
    ...
    if (baseStyle()->property("DPIAware").toBool())
    {
        QRect rectImage(pos, rcSrc.size());
        rectImage.setWidth(static_cast<int>(DrawHelpers::dpiScaled(static_cast<qreal>(rcSrc.width()))));
        rectImage.setHeight(static_cast<int>(DrawHelpers::dpiScaled(static_cast<qreal>(rcSrc.height()))));

        bool wasSmoothPixmapTransform = p->renderHints() & QPainter::SmoothPixmapTransform;
        p->setRenderHint(QPainter::SmoothPixmapTransform, true);
        p->drawPixmap(rectImage, images, rcSrc);
        p->setRenderHint(QPainter::SmoothPixmapTransform, wasSmoothPixmapTransform);
    }
    else
...
static QPixmap qtn_scaleImage(const QPixmap& px)
{
    int count = 43;
    QSize size(9 * count, 9);
...
//    return QPixmap::fromImage(img);
    return qtn_scaleImage(QPixmap::fromImage(img));
}
...
{
    m_ImagesSrc = QPixmap(":/res/standard_images.png");
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
...
bool OfficePaintManager2016::drawComboBox(const QStyleOptionComplex* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionComboBox* cmb = qstyleoption_cast<const QStyleOptionComboBox*>(opt))
    {
        ...
        const bool onlyClrAccent = theme() == OfficeStyle::Office2016White;

        QColor colorBorder = d.m_clrEditCtrlBorder;
        if (!enabled && theme() != OfficeStyle::Office2016Black)
            colorBorder = m_clrTextDisabled;
        else if (selected || dropped)
        {
            if (theme() == OfficeStyle::Office2016Black)
        ...
        else if (enabled && (highlighted || hasFocus))
        {
            if (theme() == OfficeStyle::Office2016Black)
                colorBorder = qRgb(140, 140, 140);
            else
