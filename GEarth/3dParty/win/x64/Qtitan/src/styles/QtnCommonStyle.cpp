...
void CommonStylePrivate::setPalette(QWidget* widget)
{
    ...
    connect(widget, SIGNAL(destroyed(QObject*)), this, SLOT(destroyedWidget(QObject*)));
}
...
void CommonStylePrivate::unsetPalette(QWidget* widget)
{
    if (m_customPaletteWidgets.contains(widget)) 
    {
        ...
        disconnect(widget, SIGNAL(destroyed(QObject*)), this, SLOT(destroyedWidget(QObject*)));
    }
}

void CommonStylePrivate::destroyedWidget(QObject* object)
{
    if (QWidget* wd = qobject_cast<QWidget*>(object))
    {
        if (m_customPaletteWidgets.contains(wd))
        {
            m_customPaletteWidgets.remove(wd);
            disconnect(wd, SIGNAL(destroyed(QObject*)), this, SLOT(destroyedWidget(QObject*)));
        }
    }
}
    ...
    if (!isNativeDialog(widget))
    {
        switch(pe)
        {
            ...
            case PE_FrameTabBarBase : // draw a line along the tabs
                    ret = drawFrameTabBarBase(option, p, widget);
                break;
...
QIcon CommonStyle::standardIcon(QStyle::StandardPixmap standardIcon, const QStyleOption* opt, const QWidget* widget) const
{
    ...
    if (!isNativeDialog(widget))
    {
        QIcon icon = modifyIcon(standardIcon, opt, widget, ret);
        if (ret && !icon.isNull())
            return icon;
    }
...
/*! \internal */
bool CommonStyle::canWordWrap(const QRect& rect, const QFontMetrics& fm, const QString& str, int* heightLine) const
{
                                 ...
                                 QPainter* p, const QWidget* widget, int iDurationOver) const
{
    ...
    if (!transitionsEnabled() && widget && widget->isVisible())
        return false;
