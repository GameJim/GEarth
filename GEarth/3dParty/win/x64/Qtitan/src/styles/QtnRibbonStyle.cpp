...
/*!
    ...
    \value PE_Backstage
    \value PE_RibbonBackstageCloseButton
*/
        ...
        case CC_ToolButton:
            if (const QStyleOptionToolButton* tb = qstyleoption_cast<const QStyleOptionToolButton*>(opt)) 
            {
                ...
                case SC_ToolButtonMenu:
                    if (qobject_cast<const RibbonBackstageButton*>(widget))
                    {
                        ret = CommonStyle::subControlRect(cc, opt, sc, widget);
                    }
                    else if (!qobject_cast<const RibbonGroupPopupButton *>(widget))
                    {
                        if (tb->toolButtonStyle == Qt::ToolButtonTextUnderIcon) 
                        {
    ...
    if (const StyleOptionTitleBar* optTitleBar = qstyleoption_cast<const StyleOptionTitleBar*>(opt))
    {
        if (optTitleBar->frameHelper)
        {
            ...
            if (!dwmEnabled)
            {
                ...
                Qt::Alignment fl = Qt::AlignCenter;
...
void RibbonStyle::drawFileButtonLabel(const QStyleOption* option, QPainter* p, const QWidget* widget) const
{
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(option)) 
    {
        ...
        if (proxy()->styleHint((QStyle::StyleHint)RibbonStyle::SH_RibbonItemUpperCase, option, widget))
            text = text.toUpper();
    ...
    if (qobject_cast<const RibbonPageSystemPopup*>(widget))
    {
        if (const QStyleOptionMenuItem* menuitem = qstyleoption_cast<const QStyleOptionMenuItem*>(opt)) 
        {
            ...
            else
            {
                ...
                const int nIconSize = proxy()->pixelMetric(PM_SmallIconSize, opt, widget);
