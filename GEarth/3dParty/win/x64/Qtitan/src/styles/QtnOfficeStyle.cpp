...
void OfficeStyle::polish(QWidget* widget)
{
    ...
    if (OfficePopupWindow* popupWindow = qobject_cast<OfficePopupWindow*>(widget))
    {
        if (QWidget* central = popupWindow->centralWidget())
        {
            QPalette palette = central->palette();
            QColor clrNormalText = helper().getColor("Dialog", "Foreground");
            palette.setColor(QPalette::All, QPalette::WindowText, clrNormalText);
    ...
    {
        QPalette palette = widget->palette();
        palette.setColor(QPalette::Normal, QPalette::ButtonText, helper().getColor("Window", "BarText"));
        widget->setPalette(palette);
    }
    else if (qobject_cast<QGroupBox*>(widget))
    {
        QPalette palette = widget->palette();
        palette.setColor(QPalette::Normal, QPalette::Foreground, helper().getColor("Window", "BarText"));
        widget->setPalette(palette);
    }
        ...
        case PopupSystemDecoration :
            {
                ...
                switch (getTheme())
                {
                    ...
                    case OfficeStyle::Office2013Gray :
                        ...
                        break;
                    case OfficeStyle::Office2013Dark :
                            popup2003Style->setDecoration(PopupOffice2003DrawHelper::OS_SYSTEMDARK);
                        break;
                    case OfficeStyle::Office2016Colorful:
                    case OfficeStyle::Office2016White:
                            popup2003Style->setDecoration(PopupOffice2003DrawHelper::OS_SYSTEMWHITE);
                        break;
                    case OfficeStyle::Office2016DarkGray:
                            popup2003Style->setDecoration(PopupOffice2003DrawHelper::OS_SYSTEMDARKGRAY);
                        break;
                    case OfficeStyle::Office2016Black:
                            popup2003Style->setDecoration(PopupOffice2003DrawHelper::OS_SYSTEMBLACK);
                        break;
                    default:
                            Q_ASSERT(false);
                        break;
...
*/
    ...
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        if (((!hasArrow && toolbutton->icon.isNull()) && !toolbutton->text.isEmpty())|| toolbutton->toolButtonStyle == Qt::ToolButtonTextOnly) 
        {
            ...
            if (proxy()->styleHint((QStyle::StyleHint)RibbonStyle::SH_RibbonItemUpperCase, opt, w))
                text = text.toUpper();
        ...
        else 
        {
            ...
            if (qobject_cast<const RibbonButton*>(w) || qobject_cast<const PopupColorButton*>(w))
            {
            ...
            else
            {
                if (toolbutton->toolButtonStyle != Qt::ToolButtonIconOnly) 
                {
                    ...
                    if (toolbutton->toolButtonStyle == Qt::ToolButtonTextUnderIcon) 
                    {
                        ...
                        if (!hasArrow)
                            ...
                            drawArrow(proxy(), toolbutton, pr, p, w);
                ...
                {
                    if (hasArrow)
                        drawArrow(proxy(), toolbutton, rect, p, w);
                    else
...
bool OfficeStyle::drawMdiControls(const QStyleOptionComplex* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (opt->subControls & QStyle::SC_MdiCloseButton)
    {
        ...
        pm = invertPixmap(pm, getTextColor((opt->state & State_MouseOver)&&(opt->activeSubControls & QStyle::SC_MdiCloseButton), 
            false, opt->state & State_Enabled, false, false, TypeMenuBar, BarNone));
