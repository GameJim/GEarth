...
void RibbonBackstageViewMenu::paintEvent(QPaintEvent* event)
{
    ...
    style()->drawPrimitive((QStyle::PrimitiveElement)RibbonStyle::PE_Backstage, &optBackstage, &painter, m_backstageView);
    ...
    for (int i = 0, count = acts.count(); i < count; ++i) 
    {
        ...
        painter.save();
#if 0
        painter.setClipRegion(emptyArea1);
#endif
        style()->drawControl(QStyle::CE_MenuItem, &opt, &painter, m_backstageView);
        painter.restore();
    }
}
...
QSize RibbonBackstageButton::sizeHint() const
{
    ...
    if (opt.toolButtonStyle != Qt::ToolButtonIconOnly)
    {
        if (opt.toolButtonStyle == Qt::ToolButtonTextUnderIcon)
        {
            ...
            sz.setWidth(sizeWordWrap.width());
            return sz;
        }
        else if (opt.toolButtonStyle == Qt::ToolButtonTextOnly)
        {
            ...
            sz.setWidth(sizeWordWrap.width() + opt.fontMetrics.width('X'));
            return sz;
        }
    }
...
void RibbonBackstageButton::paintEvent(QPaintEvent* event)
{
        ...
        option.toolButtonStyle == Qt::ToolButtonTextOnly)
    {
        ...
        const int indentation = option.icon.isNull() ? /*(option.fontMetrics.height() / 2) / 2*/0 : 
        ...
        ::qtn_QTextDocument_draw(p, d.m_document, option.palette, rcTxt);
