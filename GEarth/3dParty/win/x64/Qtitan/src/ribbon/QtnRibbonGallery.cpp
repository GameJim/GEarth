...
void RibbonGalleryPrivate::layoutItems()
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        if (item->isSeparator())
        {
            ...
            const int marginH = (p.style()->pixelMetric(QStyle::PM_FocusFrameHMargin) + 1) * 2;
            const int heightLabel = qfm.boundingRect(item->caption().isEmpty() ? QString("XX") : item->caption()).height() + marginH;
...
void RibbonGalleryPrivate::layoutScrollBar()
{
    ...
    else if (m_buttonPopup && m_buttonScrollUp && m_buttonScrollDown)
    {
        ...
        int width = p.style()->pixelMetric(QStyle::PM_ScrollBarExtent, 0, &p);
        rectScroll.setLeft(rectScroll.right() - width);
        
        int height = qRound((qreal)rectScroll.height() / 3.0);
...
void RibbonGalleryItem::setSizeHint(const QSize& size)
{
    ...
    if (QApplication::style()->property("DPIAware").toBool())
        sz = QSize(static_cast<int>(DrawHelpers::dpiScaled(static_cast<qreal>(size.width()))), 
            static_cast<int>(DrawHelpers::dpiScaled(static_cast<qreal>(size.height()))));
    setData(Qt::SizeHintRole, sz);
}
...
void RibbonGalleryItem::draw(QPainter* p, RibbonGallery* gallery, QRect rectItem, bool enabled, bool selected, bool pressed, bool checked)
{
    ...
    if (OfficeStyle* style = qobject_cast<OfficeStyle*>(gallery->style()) )
    {
        ...
        {
            QRect rect = rectItem;
            style->drawRectangle(p, rect, selected, pressed, enabled, checked, false, TypeNormal, BarPopup);

            QList<QSize> lstSz = icon().availableSizes(enabled ? QIcon::Normal : QIcon::Disabled);
            QSize iconSize = lstSz.size() > 0 ? lstSz[0] : rectItem.size();

            if (QApplication::style()->property("DPIAware").toBool())
                iconSize = QSize(static_cast<int>(DrawHelpers::dpiScaled(static_cast<qreal>(iconSize.width()))),
                    static_cast<int>(DrawHelpers::dpiScaled(static_cast<qreal>(iconSize.height()))));

            QPixmap pm = icon().pixmap(iconSize, enabled ? QIcon::Normal : QIcon::Disabled);
            if (!pm.isNull())
            {
...
#endif
                int space = (int)DrawHelpers::dpiScaled(static_cast<qreal>(4));
...
QSize RibbonGalleryGroup::size() const
{
    QTN_D(const RibbonGalleryGroup);
    if (QApplication::style()->property("DPIAware").toBool())
        return QSize(static_cast<int>(DrawHelpers::dpiScaled(static_cast<qreal>(d.m_sizeItem.width()))),
            static_cast<int>(DrawHelpers::dpiScaled(static_cast<qreal>(d.m_sizeItem.height()))));
    else
        return d.m_sizeItem;
}
...
void RibbonGallery::setBorderVisible(bool showBorders)
{
    QTN_D(RibbonGallery);
    if (d.m_showBorders != showBorders)
    {
        d.m_showBorders = showBorders;
        update();
    }
}
...
void RibbonGallery::paintEvent(QPaintEvent*)
{
    ...
    if (!parentWidget()->property(_qtn_PopupBar).toBool() && isBorderVisible())
    {
...
void RibbonGallery::wheelEvent(QWheelEvent* event)
{
    ...
    if (qobject_cast<OfficePopupMenu*>(parentWidget()))
        d.setScrollPos(d.m_scrollPos + (event->delta() < 0 ? 3 * group->size().height() : -3 * group->size().height()));
}
