...
void CustomizeEngineAbstract::unsetActionId(QAction* action)
{
    if (action == Q_NULL || action->isSeparator())
        return;
...
void CustomizeEngineAbstract::removeFromCategory(const QString& category, QAction* action)
{
    if (category.isEmpty() || action == Q_NULL || action->isSeparator())
        return;
...
void RibbonBarCustomizeEngine::removeFromCategory(const QString& category, QAction* action)
{
    CustomizeEngineAbstract::removeFromCategory(category, action);
}
...
{
    QTN_D(RibbonCustomizeManager);
    d.m_ribbonManager->removeFromCategory(category, action);
}
