...
{
    QTN_P(RibbonGroup);
    m_optionButton->setVisible(p.isOptionButtonVisible() && !p.isReduced());
...
{
    // GBMP-4870 解决小分辨率下崩溃的额外措施
    // by wangzy-g 2017-09-21 

    //Q_ASSERT(canReduce());
    RibbonControlSizeDefinition::GroupSize newSize = 
        (RibbonControlSizeDefinition::GroupSize)(canReduce() ? (m_currentSize + 1) : m_currentSize);

    setReductionSize(newSize);
}
...
RibbonGroupPopup* RibbonGroupPrivate::groupPopupWidget()
{
    QTN_P(RibbonGroup);
    if (m_groupPopupWidget == Q_NULL)
    {
        m_groupPopupWidget = new RibbonGroupPopup(&p);
...
/*!//[GCMP]
Insert the ribbon \a control to the group.
*/
void RibbonGroup::insertControl(RibbonControl* beforeControl, RibbonControl* control)
{
    QTN_D(RibbonGroup);
    int index = d.m_controls.indexOf(beforeControl);
    if (d.m_controls.indexOf(control) != -1)
        return; // Already was added.
    if (index > d.m_controls.size() - 1)
        return;

    control->qtn_d().m_parentGroup = this;
    d.m_controls.insert(index, control);
    control->setFont(font());
    control->sizeChanged(currentSize());
    d.updateLayoutParent();
}//[END GCMP]
...
void RibbonGroup::paintEvent(QPaintEvent* event)
{
    ...
    if (isReduced())
    {
        ...
        if (d.groupPopupWidget()->isVisible())
    ...
    else
    {
        ...
        rcEntryCaption.adjust(4, -1, -indent - 4, -2);
        opt.text = p.fontMetrics().elidedText(opt.text, d.titleTextElideMode(), rcEntryCaption.width());
...
void RibbonGroup::mousePressEvent(QMouseEvent* event)
{
    ...
    if (isReduced())
    {
        if (event->button() == Qt::LeftButton) 
        {
            ...
            d.groupPopupWidget()->popup(pnt, 0);
            return;
        }
    }
