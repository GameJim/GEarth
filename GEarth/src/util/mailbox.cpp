...
namespace util {
    ...
    void Mailbox::open(Scheduler& scheduler_) {
        ...
        if (!queue.empty()) {
            ...
            if (guard) guard->schedule(makeClosure(shared_from_this()));
        }
    }
    ...
    bool Mailbox::isOpen() const {
        return weakScheduler.lock() ? true:false;
    }
    ...
    void Mailbox::push(std::unique_ptr<Message> message) {
        ...
        auto guard = weakScheduler.lock();
        if (wasEmpty) {
            if (auto pScheduler = weakScheduler.lock())
            {
                pScheduler->schedule(makeClosure(shared_from_this()));
            }
    ...
    void Mailbox::receive() {
        ...
        //优先往任务池分发任务
        if (!wasEmpty) {
            if (auto locked = weakScheduler.lock()) {
                locked->schedule(makeClosure(shared_from_this()));
            }
        }
